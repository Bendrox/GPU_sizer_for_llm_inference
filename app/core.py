...
def calculer_kv_cache(p: ModeParams, length_seq: int, include_model_weights: bool = False, batch_size: int = 1) -> KVCacheResult:
    ...
    return KVCacheResult(
        memory_consumption_fp32_mo=oct_to_mo(elements * 4 + model_weights),
        memory_consumption_bf16_mo=oct_to_mo(elements * 2 + model_weights),
        memory_consumption_fp8_mo=oct_to_mo(elements * 1 + model_weights),
